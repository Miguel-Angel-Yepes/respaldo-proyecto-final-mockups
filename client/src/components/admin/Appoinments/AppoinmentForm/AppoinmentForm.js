...
import { initialValues, validationSchema } from './AppoinmentForm.form';
...
export function AppoinmentForm() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue, { resetForm }) => {
      ...
      try {
        // Cambiar el método a createAppoinmentByAdmin
        const result = await appoinmentController.createAppoinmentByAdmin(formValue, accessToken);
        setModalContent(result.msg); // Mensaje del backend
        setModalOpen(true);
        resetForm();
      } catch (error) {
        setModalContent('Error al crear la cita: ' + (error.msg || error.message)); // Mostrar el mensaje de error del backend
        console.error(error);
        setModalOpen(true);
      }
    },
      ...
      <div className={styles.formWrapper}>
        ...
        <Form className={styles.formElement} onSubmit={formik.handleSubmit}>
          ...
          <Form.Dropdown
            ...
            onChange={(_, data) => {
              formik.setFieldValue('appoinmentType', data.value);
            }}
            className={styles.formElement}
